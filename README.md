...
- 查看每个任务的脱敏诊断时间线，定位队列、上下文、Agent、产物和失败阶段
...
每个任务另有持久化诊断日志，记录入队、worker 领取、上下文统计、Agent 阶段、候选产物、
确认和失败。日志只接受字段白名单：事件码、组件、Agent 名、来源数量、检索数量、字符数、
耗时、token/费用、产物文件名与稳定错误码；不会保存正文、完整 Prompt、模型原始响应、
API Key、Authorization 或供应商原始错误。运行记录页支持按级别查看和复制安全诊断摘要。
...
运行期作品数据位于 `TAME_INK_WORKSPACE` 指定目录；未设置时使用当前目录下的 `.tame-ink-workspace`。正式内容位于项目 `canon/`，未确认内容位于 `.tame-ink/drafts/`，SQLite 保存任务、SSE 事件和诊断日志，索引可从正式文件重建。API 默认只监听 `127.0.0.1`。
