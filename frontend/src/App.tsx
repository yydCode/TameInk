...
function App() {
  ...
  const [taskRun, setTaskRun] = useState<TaskRunManifest | null>(null);
  ...
  function refreshWorkflowStatus(projectId = project?.id) {
    if (!projectId) return;
    getWorkflowStatus(projectId).then(setWorkflowStatus).catch(() => setWorkflowStatus(null));
  }
  ...
  useEffect(() => {
    ...
    getWorkflowStatus(activeProjectId).then(setWorkflowStatus).catch(() => setWorkflowStatus(null));
  }, [activeProjectId]);
  ...
  useEffect(() => {
    ...
    let active = true;
    setTaskRun(null);
    getTaskRun(activeProjectId, activeTaskId).then(
      (manifest) => {
        if (active) setTaskRun(manifest);
      },
      () => {
        if (active) setTaskRun(null);
      },
    );
    return () => {
      active = false;
  ...
  async function confirmCurrent() {
    ...
    try {
      ...
      setVersionConflict(false);
      refreshWorkflowStatus(project.id);
    } catch (cause) {
      ...
      <div className="app-body">
        ...
        <main className="main-content">
          ...
          {!project && view === "settings" ? <><div className="content-header"><div><div className="eyebrow">模型设置</div><h2>模型设置</h2></div></div><div className="workspace-grid"><section className="editor-panel"><SettingsView onError={setError} /></section></div></> : !project ? <ProjectRequiredView view={view as Exclude<View, "settings">} projects={projects} onCreate={() => setShowCreate(true)} onOpen={openProject} /> : <>
            ...
            <div className="workspace-grid"><section className="editor-panel">{view === "overview" && <Overview project={project} workflow={workflowStatus} onOpen={() => setView("chapters")} onNavigate={setView} />}{view === "commercial" && <CommercialView project={project} task={activeTask} workflow={workflowStatus} onNavigate={setView} onTask={setActiveTask} onWorkflowChanged={() => refreshWorkflowStatus()} onError={setError} />}{view === "imports" && <ImportView project={project} onTask={setActiveTask} onError={setError} />}{view === "story" && <StoryView project={project} workflow={workflowStatus} onNavigate={setView} draft={draft} documentPath={draftPath} setDraft={setDraft} task={activeTask} onApprove={confirmCurrent} onGenerated={(task, path, content, baseline) => { setActiveTask(task); setDraftPath(path); setOriginalDraft(baseline); setDraft(content); }} onOpenChapters={() => setView("chapters")} onError={setError} />}{view === "chapters" && <ChapterView project={project} workflow={workflowStatus} onNavigate={setView} draft={draft} setDraft={setDraft} task={activeTask} audit={chapterAudit} onAudit={setChapterAudit} onSave={persistCurrentDraft} onGenerated={(task, content, audit) => { setActiveTask(task); setDraftPath("chapter.md"); setOriginalDraft(task.status === "completed" ? content : ""); setDraft(content); setChapterAudit(audit); }} onError={setError} />}{view === "memory" && <MemoryView project={project} />}{view === "runs" && <RunsView task={activeTask} project={project} onTask={setActiveTask} onError={setError} />}{view === "settings" && <SettingsView onError={setError} />}</section><aside className="context-panel"><ContextPanel project={project} workflow={workflowStatus} task={activeTask} run={taskRun} dirty={isDirty} onReview={() => setShowDiff(true)} onNavigate={setView} /></aside></div>
          </>}
        </main>
      </div>
...
type WorkflowStep = {
  id: "setting" | "outline" | "volume" | "commercial";
  label: string;
  description: string;
  view: View;
  complete: boolean;
...
function workflowSteps(workflow: WorkflowStatus | null): WorkflowStep[] {
  return [
    { id: "setting", label: "确认故事设定", description: "明确核心冲突、主角目标和世界规则。", view: "story", complete: Boolean(workflow?.setting_confirmed) },
    { id: "outline", label: "确认全书大纲", description: "确认主线、阶段目标和关键转折。", view: "story", complete: Boolean(workflow?.outline_confirmed) },
    { id: "volume", label: "确认第一卷", description: "确认首卷节奏、冲突升级与收束。", view: "story", complete: Boolean(workflow?.volume_one_confirmed) },
    { id: "commercial", label: "确认商业定位", description: "确认读者承诺与首测策略。", view: "commercial", complete: Boolean(workflow?.commercial_confirmed) },
  ];
}

function nextWorkflowStep(workflow: WorkflowStatus | null): WorkflowStep | null {
  return workflowSteps(workflow).find((step) => !step.complete) ?? null;
}

function workflowReadyForChapters(workflow: WorkflowStatus | null): boolean {
  return workflow !== null && workflowSteps(workflow).every((step) => step.complete);
}

function WorkflowGuide({ workflow, onNavigate, compact = false }: { workflow: WorkflowStatus | null; onNavigate: (view: View) => void; compact?: boolean }) {
  const steps = workflowSteps(workflow);
  const next = nextWorkflowStep(workflow);
  if (compact) return <div className="workflow-compact"><span className="context-label">创作流程</span><span>{workflow ? `${steps.filter((step) => step.complete).length} / ${steps.length} 已确认` : "正在读取确认状态"}</span>{next && <button type="button" onClick={() => onNavigate(next.view)}>下一步：{next.label}</button>}</div>;
  return <section className="workflow-guide" aria-label="创作流程引导"><div><span className="eyebrow">创作流程</span><h3>{next ? `下一步：${next.label}` : "章节生成已解锁"}</h3><p>{next ? `${next.description} 草稿生成后仍需由你确认，确认后才会解锁下一步。` : "四项前置内容均已确认，现在可以开始生成章节。"}</p></div><ol>{steps.map((step, index) => <li key={step.id} className={step.complete ? "is-complete" : step.id === next?.id ? "is-current" : ""}><span>{step.complete ? <Check size={13} /> : index + 1}</span><div><strong>{step.label}</strong><small>{step.complete ? "已确认" : step.id === next?.id ? "当前需要完成" : "等待前置步骤"}</small></div></li>)}</ol>{next && <button className="button button-primary" type="button" onClick={() => onNavigate(next.view)}>前往{next.view === "commercial" ? "商业增长" : "故事设计"}<ChevronRight size={15} /></button>}</section>;
}

function Overview({ project, workflow, onOpen, onNavigate }: { project: Project; workflow: WorkflowStatus | null; onOpen: () => void; onNavigate: (view: View) => void }) { return <div className="overview-view"><div className="welcome-band"><div><span className="eyebrow">作品概览</span><h3>{project.title}</h3><p>{project.constraints ?? "尚未填写创作约束"}</p></div><button className="button button-secondary" type="button" onClick={onOpen}>打开章节工作台<ChevronRight size={15} /></button></div><div className="metric-grid"><Metric label="目标字数" value={project.target_words ? `${Math.round(project.target_words / 10000)} 万字` : "未设置"} /><Metric label="当前章节" value="尚未开始" /><Metric label="待处理伏笔" value="0" /></div><WorkflowGuide workflow={workflow} onNavigate={onNavigate} /></div>; }
function Metric({ label, value }: { label: string; value: string }) { return <div className="metric"><span>{label}</span><strong>{value}</strong></div>; }

function StoryView({ project, workflow, onNavigate, draft, documentPath, setDraft, task, onApprove, onGenerated, onOpenChapters, onError }: { project: Project; workflow: WorkflowStatus | null; onNavigate: (view: View) => void; draft: string; documentPath: string; setDraft: (value: string) => void; task: Task | null; onApprove: () => void; onGenerated: (task: Task, path: string, content: string, baseline: string) => void; onOpenChapters: () => void; onError: (message: string) => void }) {
...
function CommercialView({ project, task, workflow, onNavigate, onTask, onWorkflowChanged, onError }: { project: Project; task: Task | null; workflow: WorkflowStatus | null; onNavigate: (view: View) => void; onTask: (task: Task) => void; onWorkflowChanged: () => void; onError: (message: string) => void }) {
...
function ChapterView({ project, workflow, onNavigate, draft, setDraft, task, audit, onAudit, onSave, onGenerated, onError }: { project: Project; workflow: WorkflowStatus | null; onNavigate: (view: View) => void; draft: string; setDraft: (value: string) => void; task: Task | null; audit: CommercialAudit | null; onAudit: (audit: CommercialAudit | null) => void; onSave: () => Promise<void>; onGenerated: (task: Task, content: string, audit: CommercialAudit | null) => void; onError: (message: string) => void }) {
  ...
  const next = nextWorkflowStep(workflow);
  const chapterReady = workflowReadyForChapters(workflow);
  return <div className="editor-view"><WorkflowGuide workflow={workflow} onNavigate={onNavigate} />{!chapterReady && next && <section className="workflow-callout" aria-label="章节生成前置条件"><div><strong>章节生成尚未解锁</strong><span>请先{next.label}。只有正式确认的内容会作为章节 Agent 的上下文。</span></div><button className="button button-secondary" type="button" onClick={() => onNavigate(next.view)}>前往完成<ChevronRight size={15} /></button></section>}<div className="section-heading"><div><span className="eyebrow">逐章创作</span><label className="chapter-number-label" htmlFor="chapter-id">第</label><input id="chapter-id" className="chapter-number" value={chapterId} onChange={(event) => setChapterId(event.target.value.replace(/\D/g, ""))} inputMode="numeric" aria-label="章节编号" /><span className="chapter-number-label">章</span></div><div className="header-actions">{task?.status === "awaiting_approval" && <button className="button button-secondary" type="button" onClick={auditCurrent} disabled={busy}><Target size={15} />重新商业审查</button>}{task?.status === "awaiting_approval" && <button className="button button-secondary" type="button" onClick={approve} disabled={busy || (overrideRequired && !overrideReason.trim())}><Check size={15} />{overrideRequired ? "覆盖门禁并确认" : "确认章节"}</button>}<button className="button button-primary" type="button" onClick={start} disabled={busy || !chapterReady} aria-describedby={!chapterReady ? "chapter-workflow-hint" : undefined}>{busy ? <LoaderCircle className="spin" size={15} /> : <Sparkles size={15} />}Agent 生成章节</button></div></div>{!chapterReady && <p id="chapter-workflow-hint" className="workflow-button-hint">完成引导中的四项确认后即可生成章节。</p>}{audit && <section className={`commercial-audit ${audit.commercial_gate_passed ? "is-pass" : "is-blocked"}`} aria-label="商业质量审查"><div className="commercial-audit-heading"><div><span className="eyebrow">留存审查</span><strong>{audit.commercial_report.total_score} / 100</strong></div><span>{audit.commercial_gate_passed ? "达到确认门槛" : `低于 ${audit.minimum_commercial_score} 分门槛`}</span></div><div className="commercial-score-grid">{audit.commercial_report.dimensions.map((dimension) => <div key={dimension.dimension}><span>{commercialDimensionLabels[dimension.dimension]}</span><progress max="100" value={dimension.score} /><strong>{dimension.score}</strong></div>)}</div>{audit.commercial_report.issues.length > 0 && <div className="commercial-issues">{audit.commercial_report.issues.map((issue) => <div key={issue.id}><strong>{commercialDimensionLabels[issue.dimension]}</strong><span>{issue.description}</span><q>{issue.citation.quote}</q></div>)}</div>}{overrideRequired && <label className="commercial-override">人工覆盖理由<textarea value={overrideReason} onChange={(event) => setOverrideReason(event.target.value)} placeholder="说明为什么该章可以低于门槛进入正式稿" /></label>}</section>}<label className="field-label" htmlFor="chapter-plan">Agent 指令</label><textarea id="chapter-plan" className="plan-input" value={instruction} onChange={(event) => setInstruction(event.target.value)} /><NovelEditor markdown={draft} onChange={setDraft} /><div className="editor-footer"><span>当前任务：{task ? task.status : "尚未启动"}</span><span>项目：{project.id}</span></div></div>;
}
...
function ContextPanel({ project, workflow, task, run, dirty, onReview, onNavigate }: { project: Project; workflow: WorkflowStatus | null; task: Task | null; run: TaskRunManifest | null; dirty: boolean; onReview: () => void; onNavigate: (view: View) => void }) {
  const latest = run?.agent_runs.at(-1);
  return <><div className="panel-heading"><div><span className="eyebrow">Agent 面板</span><h3>上下文</h3></div><Search size={16} /></div><div className="context-block"><span className="context-label">当前项目</span><strong>{project.title}</strong><span className="muted">{project.genre ?? "未设置题材"}</span></div><div className="context-block"><WorkflowGuide workflow={workflow} onNavigate={onNavigate} compact /></div><div className="context-block"><span className="context-label">任务状态</span>{task ? <RunStatus status={task.status} connection="connected" /> : <span className="muted">暂无活动任务</span>}</div><div className="context-block"><span className="context-label">运行上下文</span>{latest ? <><strong>{latest.stage}</strong><span className="agent-run-skill">{latest.agent} · {latest.skill}</span><div className="context-stats"><span>{run?.agent_runs.length ?? 0} 次 Agent</span><span>{latest.queries.length} 条检索</span><span>{latest.total_characters.toLocaleString("zh-CN")} 字符</span></div><small className="skill-hash" title={latest.skill_sha256}>Skill SHA {latest.skill_sha256.slice(0, 12)}</small></> : <span className="muted">当前任务没有 Agent 上下文记录</span>}</div><div className="context-block"><span className="context-label">版本门禁</span><div className="gate-row"><span className={`gate-dot ${dirty ? "gate-dot--warn" : ""}`} />{dirty ? "等待用户确认" : "正式内容未修改"}</div>{dirty && <button className="button button-secondary" type="button" onClick={onReview}><GitCompareArrows size={14} />查看差异</button>}</div><div className="source-list"><span className="context-label">当前阶段来源</span>{latest ? latest.source_paths.map((path) => <span key={path} title={path}>{path}</span>) : <span>无</span>}</div></>;
}
