...
def new_task(project_id: str = "story-01", kind: TaskKind = TaskKind.WRITE) -> Task:
...
def test_transition_and_event_append_are_atomic(tmp_path: Path) -> None:
    ...
    updated = tasks.transition(
        task.id, TaskStatus.PENDING, TaskStatus.RUNNING, "task.started"
    )
...
def test_interrupted_write_task_still_blocks_new_write_task(tmp_path: Path) -> None:
    ...
    tasks.transition(
        first.id, TaskStatus.RUNNING, TaskStatus.INTERRUPTED, "task.interrupted"
    )
...
def test_repository_maps_database_transition_guard_to_domain_error(tmp_path: Path) -> None:
    ...
    with pytest.raises(InvalidTaskTransitionError):
        tasks.transition(
            task.id, TaskStatus.PENDING, TaskStatus.COMPLETED, "task.completed"
        )
